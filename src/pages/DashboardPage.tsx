...
import { Users, CalendarCheck, LineChart, MessageSquare, CalendarDays, ShoppingCart, Loader2, BadgeDollarSign, Scale, CalendarClock, CalendarHeart, Repeat, TagIcon, TriangleAlert, ChevronLeft, ChevronRight } from "lucide-react";
import { useQuery } from "@tanstack/react-query";
import { endOfMonth, getDay, isAfter, startOfMonth, format, subMonths, addMonths, isBefore, startOfDay } from 'date-fns';
...
interface DetailedSalesData {
    total: TotalSalesData | null;
    rebuy: RebuySalesData | null;
    new: NewSalesData | null;
}
...
const DashboardPage: React.FC<DashboardPageProps> = ({ clinicData }) => {
        ...
        queryFn: async () => {
            ...
            try {
                ...
                if (Array.isArray(data) && data.length > 0 && data[0] && typeof data[0] === 'object' && data[0].count_remoteJid !== undefined) {
                    return { count_remoteJid: Number(data[0].count_remoteJid) } as LeadsData;
        ...
        queryFn: async () => {
            ...
            try {
                ...
                if (!contentType || !contentType.includes('application/json')) {
                    ...
                    return { sum_total_agendamentos: 0, sum_total_realizados: 0 };
                }
                ...
                console.log("DashboardPage: Checking appointment keys. agendamentos:", agendamentos, "realizados:", realizados);
                console.log("DashboardPage: Type of agendamentos:", typeof agendamentos, "Type of realizados:", typeof realizados);
                console.log("DashboardPage: Result object keys:", Object.keys(result));

                if (agendamentos === undefined || realizados === undefined) {
                     console.error("DashboardPage: Keys are undefined despite data log. Result object:", result);
                ...
                // Ensure they can be converted to numbers
                 ...
                 console.log("DashboardPage: Checking appointment values. numAgendamentos:", numAgendamentos, "numRealizados:", numRealizados);

                 if (isNaN(numAgendamentos) || isNaN(numRealizados)) {
                     console.error("DashboardPage: Values are NaN despite data log. Agendados:", agendamentos, "Realizados:", realizados);
    ...
    const totalAverageTicket = (salesData?.total?.count_id_north ?? 0) > 0
        ? (salesData?.total?.sum_valor_venda ?? 0) / (salesData?.total?.count_id_north ?? 1)
        : 0;

    const rebuyAverageTicket = (salesData?.rebuy?.num_recompra ?? 0) > 0
        ? (salesData?.rebuy?.sum_recompra ?? 0) / (salesData?.rebuy?.num_recompra ?? 1)
        : 0;

    const newSalesAverageTicket = (salesData?.new?.num_nova_compra ?? 0) > 0
        ? (salesData?.new?.sum_nova_compra ?? 0) / (salesData?.new?.num_nova_compra ?? 1)
        : 0;
    ...
    return (
        <div className="welcome-container max-w-6xl mx-auto bg-white rounded-lg shadow-md p-6">
            ...
            <div className="bg-green-50 p-4 rounded-lg mb-6">
                ...
                {isLoadingSales ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <Card className="text-center">
                            <CardHeader className="pb-2">
                                <TagIcon className="mx-auto h-8 w-8 text-primary" />
