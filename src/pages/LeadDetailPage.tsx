...
interface LeadDetailPageProps {
  clinicData: { id: string | number | null; code: string; } | null; // Clinic data needed for queries
}
...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ clinicData }) => {
  const { leadId: leadIdParam } = useParams<{ leadId: string }>(); // Get leadId from URL params
  ...
  const { data: leadDetails, isLoading: isLoadingLead, error: fetchLeadError } = useQuery<LeadDetail | null>({
    ...
    enabled: !!leadId && !!clinicId, // Enable only if leadId and clinicId are available
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['paginatedLeads'] }); // Invalidate leads list
      queryClient.invalidateQueries({ queryKey: ['funnelLeads'] }); // Invalidate funnel leads
      navigate(-1); // Go back to the previous page (list or funnel)
    } catch (e: any) {
      ...
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-100px)] bg-gray-100 p-4">
        <Card className="w-full max-w-md text-center">
          ...
          <CardContent>
            ...
            <Button onClick={() => navigate('/dashboard/3')} className="mt-4">
  ...
  return (
    <div className="min-h-[calc(100vh-70px)] flex flex-col items-center justify-center bg-gray-100 p-6">
      <Card className="w-full max-w-2xl">
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-primary">Detalhes do Lead</CardTitle>
        </CardHeader>
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="flex flex-col items-center gap-3 mb-4">
                <Avatar className="h-24 w-24 border-2 border-primary">
                  {avatarPreviewUrl ? (
                    <img src={avatarPreviewUrl} alt="Avatar do Lead" className="object-cover" />
                  ) : (
