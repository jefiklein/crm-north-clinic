...
const MensagensConfigPage: React.FC<MensagensConfigPageProps> = ({ clinicData }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    variacao_5: '',
    prioridade: 1,
    servicos_vinculados: [] as number[], // IDs dos serviços vinculados selecionados
  });
  ...
  useEffect(() => {
    if (isEditing && messageDetails) {
      setFormData({
        ...
        servicos_vinculados: linkedServicesList ? linkedServicesList.map(s => s.id) : [],
      });
  ...
  }, [messageDetails, isEditing, initialCategoryFromUrl, linkedServicesList]);
  ...
  // Handler para mudança dos serviços vinculados (multi select)
  ...
  // Loading e erros
  const isLoading = isLoadingMessageDetails || isLoadingInstances || isLoadingLinkedServices || isLoadingAllServices;
  const error = messageDetailsError || instancesError || linkedServicesError || allServicesError;
        ...
        <CardContent>
          <form onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* Combo de serviços vinculados usando MultiSelectServices */}
              <div className="md:col-span-2">
                ...
                <MultiSelectServices
                  ...
                  selectedIds={formData.servicos_vinculados}
                  onChange={handleServicesChange}
                  disabled={isLoadingAllServices || isLoadingLinkedServices}
                />
