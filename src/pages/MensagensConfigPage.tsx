...
const MensagensConfigPage: React.FC<MensagensConfigPageProps> = ({ clinicData }) => {
    ...
    const { data: instancesList, isLoading: isLoadingInstances, error: instancesError } = useQuery<InstanceInfo[]>({
        ...
        queryFn: async () => {
            ...
            if (!Array.isArray(data)) {
                 ...
                 throw new Error("Formato de resposta inválido para lista de instâncias.");
            }
    ...
    const saveMessageMutation = useMutation({
        mutationFn: async (dataToSave: FormData) => {
            ...
            console(`[MensagensConfigPage] Saving message (${isEditing ? 'Update' : 'Create'}) to ${url}`);
    ...
    const generateAiVariationMutation = useMutation({
        ...
        onSuccess: (data) => {
            const textarea = document.getElementById(`variationText${data.slot}`) as HTMLTextAreaElement | null;
            if (textarea) {
    ...
    useEffect(() => {
        ...
        switch (category) {
            ...
            case 'Aniversário':
                ...
                if (serviceSelectionGroupEl) serviceSelectionGroupEl.style.display = 'none';
                break;
            case 'Chegou':
            case 'Liberado':
        ...
        handleTargetTypeChange(formData.para_grupo ? 'Grupo' : (formData.para_cliente ? 'Cliente' : (formData.para_funcionario ? 'Funcionário' : 'Grupo'))); // Pass current target type
    ...
    useEffect(() => {
        ...
        const viewMediaBtnEl = document.getElementById('viewMediaBtn');
        ...
        const oldDynamicMedia = document.getElementById('dynamicMediaElement');
        if (oldDynamicMedia) oldDynamicMedia.remove();
         if (viewMediaBtnEl) viewMediaBtnEl.style.display = 'none'; // Hide view button by default
    ...
    useEffect(() => {
        ...
        // If editing and details failed to load, show error and disable form
        if (isEditing && detailsError) {
             setPageError(`Erro ao carregar detalhes da mensagem: ${detailsError.message}`);
             if (saveBtn) saveBtn.disabled = true;
        } else if (!loading && !error && saveBtn) {
             // Enable save button only when all initial data is loaded without errors
    ...
    const handleSelectChange = (id: string, value: string) => {
         ...
         // Special handling for category and instance changes
         if (id === 'categoria') {
             // No need to call handleCategoryChange directly, useEffect handles it
         } else if (id === 'id_instancia') {
             // No need to call handleInstanceChange directly, useEffect handles it
        ...
        if (groupSelectionGroupEl) {
             ...
             if (shouldShowGroupSelect) {
                 ...
                 if (currentInstanceId) {
                     ...
                     const groupSelectEl = document.getElementById('groupSelect') as HTMLSelectElement | null;
                     if(groupSelectEl) {
    ...
    const handleSave = async () => {
        ...
        if (choicesServicesRef.current && document.getElementById('serviceSelectionGroup')?.style.display !== 'none') {
             try {
        ...
        else if (currentFormData.categoria !== 'Aniversário' && document.getElementById('serviceSelectionGroup')?.style.display !== 'none' && linkedServices.length === 0) { validationError = "Pelo menos um serviço deve ser vinculado (exceto para Aniversário)."; }
        ...
        if (selectedMediaFile) {
            ...
            try {
                if (saveBtn) { saveBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Enviando Mídia...'; }
        ...
        } else if (isEditing && existingMediaKey === null && document.getElementById('currentMediaPreview')?.style.display === 'none' && document.getElementById('mediaPlaceholderText')?.style.display === 'inline') {
             // Case: Was editing, had media, user cleared it (by selecting nothing or clearing input)
             console.log("[handleSave] Existing media was cleared by user.");
             fileKeyToSave = ''; // Save empty string to remove media
        } else {
             console.log("[handleSave] No new media, keeping existing key or saving empty.");
             // fileKeyToSave already holds existingMediaKey or null/empty
        }
        ...
        if (saveBtn) { saveBtn.innerHTML = `<i class="fas fa-spinner fa-spin"></i> ${isEditing ? 'Atualizando...' : 'Criando...'}`; }
    ...
    const handleClearVariationClick = (slot: number) => {
        const textarea = document.getElementById(`variationText${slot}`) as HTMLTextAreaElement | null;
        if (textarea) {
            textarea.value = '';
            updateVariationsCounter();
        }
    };
        ...
        if (groups && groups.length > 0) {
            ...
            if (groupSelectEl.options.length > 1) { // If added any valid group
                ...
                if (targetGroupId !== null) {
                    const targetValueString = String(targetGroupId);
                    if (Array.from(groupSelectEl.options).some(opt => opt.value === targetValueString)) {
                        console.log(`[populateGroupSelect] Selecting target group ID: ${targetGroupId}`);
                        groupSelectEl.value = targetValueString;
                        setFormData(prev => ({ ...prev, grupo: targetValueString })); // Update state
                    } else {
                        console.warn(`[populateGroupSelect] Target group ID '${targetGroupId}' not found.`);
                        groupSelectEl.value = ""; // Keep placeholder selected
                        setFormData(prev => ({ ...prev, grupo: '' })); // Update state
                        if(isEditing) showToast("Grupo alvo salvo não encontrado.", "warning");
                    }
                } else {
                     setFormData(prev => ({ ...prev, grupo: '' })); // Update state
                }
    ...
    const fetchAndDisplaySavedMedia = async (fileKey: string) => {
        ...
        if (!fileKey || !currentMediaPreviewEl || !mediaPlaceholderTextEl || !currentMediaInfoEl || !mediaViewLoadingEl || !mediaPreviewContainerEl) {
        ...
        mediaViewLoadingEl.style.display = 'inline-block';
        currentMediaPreviewEl.style.display = 'none';
        currentMediaPreviewEl.src = '';
        mediaPlaceholderTextEl.style.display = 'none';
        currentMediaInfoEl.style.display = 'none'; // Hide info during load
        ...
        try {
            ...
            if (fileType === 'image') {
                ...
                mediaPreviewContainerEl.insertBefore(videoElement, mediaPlaceholderTextEl);
                ...
                mediaPreviewContainerEl.insertBefore(audioElement, mediaPlaceholderTextEl);
                ...
                mediaPreviewContainerEl.insertBefore(downloadLink, mediaPlaceholderTextEl);
            ...
            // Show container if something is displayed
             if (currentMediaPreviewEl.style.display === 'block' || document.getElementById('dynamicMediaElement')) {
            ...
            if(currentMediaInfoEl) { currentMediaInfoEl.innerHTML = `<span style="color:var(--color-error);">Falha ao carregar mídia</span>`; currentMediaInfoEl.style.display = 'inline'; }
        } finally {
            mediaViewLoadingEl.style.display = 'none';
        }
    };
    ...
    const isLoading = isLoadingPage || saveMessageMutation.isLoading || uploadMediaMutation.isLoading || generateAiVariationMutation.isLoading;
    ...
    return (
        <div className="config-container max-w-6xl mx-auto p-6 bg-gray-100 min-h-screen">
            ...
            {/* Form */}
            {!isLoadingPage && !pageError && (
                <form id="messageConfigForm" onSubmit={(e) => e.preventDefault()}>
                    ...
                    <div className="form-section bg-white border border-gray-200 rounded-lg p-6 mb-6 shadow-sm">
                        ...
                        <div className="form-group">
                            ...
                            <div id="mediaPreviewContainer" className={cn("image-preview-container mt-4 p-4 border border-dashed rounded-md bg-gray-50 flex-col items-center justify-center", (!selectedMediaFile && !existingMediaKey) && 'hidden')}>
                                ...
                                {/* Dynamic element will be inserted here by fetchAndDisplaySavedMedia */}
