...
// REMOVED: import Choices from 'choices.js'; // Removed Choices.js import
import 'emoji-picker-element'; // Ensure the emoji picker element is defined
import { supabase } from '@/integrations/supabase/client'; // Import Supabase client
import MultiSelectServices from '@/components/MultiSelectServices'; // Import the new MultiSelectServices component
...
const AI_VARIATION_WEBHOOK_URL = `${N8N_BASE_URL}/webhook/225ecff5-6081-466f-a0d7-4cfe3ea2ce84`; // Corrected webhook URL
...
const MensagensConfigPage: React.FC<MensagensConfigPageProps> = ({ clinicData }) => {
    ...
    // NEW STATE: State to hold selected service IDs
    ...
    const generateAiVariationMutation = useMutation({
        ...
        onSuccess: (data) => {
            ...
            if (textarea) {
                ...
                // updateVariationsCounter(); // Update counter after setting value - REMOVED VARIATIONS SECTION
            }
    ...
    // REMOVED: Effect to initialize Choices.js
    // REMOVED: Effect to populate Choices.js and set selection
    ...
    useEffect(() => {
        if (isEditing && messageDetails) {
            ...
            // Initial population of variations counter - REMOVED VARIATIONS SECTION
    ...
    useEffect(() => {
        if (isEditing && linkedServicesList !== undefined) { // Ensure linkedServicesList has been fetched (could be empty array)
            console.log("[useEffect] Populating selectedServiceIds from linkedServicesList:", linkedServicesList);
            const ids = linkedServicesList?.map(item => item.id_servico) || [];
            setSelectedServiceIds(ids);
        } else if (!isEditing) {
             // If creating a new message, ensure selectedServiceIds is empty
    ...
    useEffect(() => {
        // Check if linkedServicesList is undefined (still loading in edit mode)
        const isLinkedServicesLoadingInEdit = isEditing && linkedServicesList === undefined && !linkedServicesError;

        const loading = isLoadingDetails || isLoadingInstances || isLoadingServices || isLoadingGroups || isLinkedServicesLoadingInEdit;
        const error = detailsError || instancesError || servicesError || groupsError || linkedServicesError;

        setIsLoadingPage(loading);
        setPageError(error ? error.message : null);

    }, [isLoadingDetails, isLoadingInstances, isLoadingServices, isLoadingGroups, isLoadingLinkedServices, detailsError, instancesError, servicesError, groupsError, linkedServicesError, isEditing, linkedServicesList]); // Added linkedServicesList to dependencies
    ...
    // Effect to update variations counter when form data changes - REMOVED VARIATIONS SECTION
    // useEffect(() => {
    ...
    // NEW HANDLER: Handle changes from MultiSelectServices component
    const handleServiceSelectionChange = (selected: number[]) => {
        console.log("[handleServiceSelectionChange] Selected service IDs:", selected);
    ...
    const handleSave = async () => {
        ...
        // Add variations - REMOVED VARIATIONS SECTION
        ...
        if (isServiceSelectionVisible) {
             linkedServices = selectedServiceIds; // Get from the new state
             dataToSave.append('servicos_vinculados', JSON.stringify(linkedServices));
             console.log("[handleSave] Linked services added from state:", linkedServices);
        } else {
    ...
    const handleClearVariationClick = (slot: number) => {
        ...
        // updateVariationsCounter is triggered by the state change effect - REMOVED VARIATIONS SECTION
    };
    ...
    // Helper to update variations counter - REMOVED VARIATIONS SECTION
    ...
    return (
        <div className="config-container max-w-6xl mx-auto p-6 bg-gray-100 min-h-screen">
            ...
            {/* Error Display */}
            {pageError && (
                <div className="error-message flex items-center gap-2 p-3 mb-4 bg-red-100 text-red-700 border border-red-300 rounded-md shadow-sm">
                    <TriangleAlert className="h-5 w-5 flex-shrink-0" />
                    <span className="text-lg font-semibold">Erro ao carregar dados: {pageError}</span>
                    {/* Add a retry button if needed */}
                </div>
            )}

            {/* Temporary Debug Sections - REMOVED DEBUG SECTION */}
            {/* {!isLoadingPage && !pageError && (
            ...
            {!isLoadingPage && !pageError && (
                <form id="messageConfigForm" onSubmit={(e) => { e.preventDefault(); handleSave(); }}> {/* Call handleSave on form submit */}
                    ...
                    <div className="form-section bg-white border border-gray-200 rounded-lg p-6 mb-6 shadow-sm">
                        ...
                        {/* Variations Section - REMOVED */}
                        {/* ... (removed variations section JSX) ... */}
                    </div>
                    ...
                    <div className="form-section bg-white border border-gray-200 rounded-lg p-6 mb-6 shadow-sm">
                        ...
                        <div className="form-group" id="serviceSelectionGroup">
                            ...
                            {/* REPLACED: Old select with Choices.js */}
                            <MultiSelectServices
                                options={servicesList || []} // Pass available services
                                selectedIds={selectedServiceIds} // Pass currently selected IDs from state
                                onChange={handleServiceSelectionChange} // Pass handler to update state
                                disabled={isLoading || isLoadingServices || isLoadingLinkedServices} // Disable based on loading states
                            />
