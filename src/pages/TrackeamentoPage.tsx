...
const TrackeamentoPage: React.FC<TrackeamentoPageProps> = ({ clinicData }) => {
    ...
    const filteredAndSortedData = useMemo(() => {
        ...
        const filtered = leadsData.filter(lead => {
            ...
            const remoteJidPhonePart = lead.remote_jid ? lead.remote_jid.split('@')[0].toLowerCase() : ''; // Use remote_jid
            const origin = lead.origem?.toLowerCase() || '';
            // Filter by name, phone (from remote_jid), or origin
            return name.includes(lowerSearchTerm) || remoteJidPhonePart.includes(lowerSearchTerm) || origin.includes(lowerSearchTerm);
        });
