...
const ClientesPage: React.FC<ClientesPageProps> = ({ clinicData }) => {
    ...
    const { data: stagesData, isLoading: isLoadingStages, error: stagesError } = useQuery<FunnelStage[]>({
        ...
        queryFn: async () => {
            ...
            console.log(`ClientesPage: Chamando webhook de etapas para funnel_id=${funnelIdForWebhook}, clinic_id=${clinicId}`);
            ...
            console.log(`ClientesPage: Resposta webhook etapas: Status=${response.status}`);
            if (!response.ok) throw new Error(`Erro ao buscar etapas: ${response.status}`);
            const data = await response.json();
            console.log("ClientesPage: Dados brutos etapas:", data);
    ...
    const { data: funnelDetailsData, isLoading: isLoadingFunnelDetails, error: funnelDetailsError } = useQuery<FunnelDetails[]>({
        ...
        queryFn: async () => {
            if (!clinicId) throw new Error("Dados da clínica não disponíveis.");
             console.log(`ClientesPage: Chamando webhook detalhes funil para funnel_id=${funnelIdForWebhook}, clinic_id=${clinicId}`);
            const response = await fetch(FUNNEL_DETAILS_WEBHOOK_URL, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ funnel_id: funnelIdForWebhook, clinic_id: clinicId }) // Use fixed funnelIdForWebhook in the body
            });
            console.log(`ClientesPage: Resposta webhook detalhes funil: Status=${response.status}`);
            if (!response.ok) throw new Error(`Erro ao buscar detalhes do funil: ${response.status}`);
            const data = await response.json();
            console.log("ClientesPage: Dados brutos detalhes funil:", data);
            ...
            if (!Array.isArray(data) || data.length === 0 || !data[0] || typeof data[0].nome_funil === 'undefined') {
                 console.warn("ClientesPage: Resposta de detalhes do funil inválida:", data);
    ...
    const { data: clientsData, isLoading: isLoadingClients, error: clientsError } = useQuery<ClientLead[]>({
        ...
        queryFn: async () => {
            ...
            console.log(`ClientesPage: Chamando webhook clientes/leads para funnel_id=${funnelIdForWebhook}, clinic_id=${clinicId}`);
            const response = await fetch(CLIENTS_WEBHOOK_URL, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ funnel_id: funnelIdForWebhook, clinic_id: clinicId }) // Use fixed funnelIdForWebhook in the body
            });
            console.log(`ClientesPage: Resposta webhook clientes/leads: Status=${response.status}`);
            if (!response.ok) {
                ...
                console.error(`ClientesPage: Erro detalhado webhook clientes/leads: ${errorDetail}`);
                throw new Error(`Erro ao buscar clientes/leads (${response.status}): ${errorDetail}`);
            }
            const data = await response.json();
            console.log("ClientesPage: Dados brutos clientes/leads:", data);
            if (!Array.isArray(data)) {
                 console.warn("ClientesPage: Resposta de clientes/leads inválida: não é um array. Recebeu:", data);
        ...
        filteredAndSortedClients.forEach(lead => {
            if (lead.id_etapa !== null && grouped[lead.id_etapa]) {
                grouped[lead.id_etapa].push(lead);
                 ...
                 console.warn(`ClientesPage: Client/Lead ${lead.id} has invalid stage ID: ${lead.id_etapa}`);
            }
        });
        return grouped;
